...
func flattenDelegatedRoutes(
	...
	if !ok {
		return eris.Errorf("Unsupported route kind %v", parent.Object.GetObjectKind().GroupVersionKind().Kind)
	}
	...
	for _, child := range children {
		...
		if !ok {
			return eris.Errorf("Unsupported route kind %v", child.Object.GetObjectKind().GroupVersionKind().Kind)
		}
